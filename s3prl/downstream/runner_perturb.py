...
class Runner():
    ...
    def __init__(self, args, config):
        ...
        self.pse = Transformer(
           d_model=768,
           nhead=12,
           num_encoder_layers=6
        ).to('cuda')
        self.pse.load_state_dict(torch.load(self.args.pse))
    ...
    def evaluate(self, split=None, logger=None, global_step=0):
        ...
        for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc=split, total=evaluate_steps)):
            ...
            with torch.no_grad():
                ...
                features_len = torch.IntTensor([len(feat) for feat in features]).to('cuda')

                features_pad = features_pad.detach().to('cuda')

                estimated_gradient = self.pse(features_pad)
                B, T, D = estimated_gradient.size()
                estimated_gradient = estimated_gradient.reshape(B, T*D)
                sorted_gradient, _ = torch.sort(estimated_gradient, dim=1)
                threshold_idx = int((self.threshold*0.2)*(T*D))
                threshold_vector = sorted_gradient[:, threshold_idx]
                
                mask_pred = (estimated_gradient.T >= threshold_vector).T.reshape(B, T, D)
                # mask_pred = torch.randint(0, 5, features_pad.size(), device='cuda') >= self.threshold # random predictions

                loc = torch.zeros(features_pad.size())
                scale = torch.ones(features_pad.size())*(2.0/self.eps)
                m = torch.distributions.laplace.Laplace(loc, scale)
                noise = m.sample().to('cuda')

                features_pad_clamped = torch.clamp(features_pad,min=-1.0,max=1.0)
                features_pad = features_pad.clone().detach().to('cuda:0')*(~mask_pred) + torch.clamp(features_pad_clamped.clone().detach().to('cuda:0')+noise, -1.0, 1.0)*(mask_pred)

                features = []
                for item_num in range(features_pad.size(0)):
                    features.append(features_pad[item_num][0:features_len[item_num]])
