        ...
        while pbar.n < pbar.total:
            ...
            for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    saliency_maps = self.downstream.model.compute_saliency_map(features=features, labels=others[0])
                    features_pad = pad_sequence(features, batch_first=True)

                    for item_num in range(len(others[0])):
                        current_dict = {}
                        current_dict['w2v2_features'] = features_pad[item_num].detach().cpu().numpy()
                        current_dict['saliency_map'] =  saliency_maps[item_num].detach().cpu().numpy()

                        with open(os.path.join(self.save_path, others[1][item_num]+'.pkl'), 'wb') as handle:
                            pickle.dump(current_dict, handle, protocol=pickle.HIGHEST_PROTOCOL)
                            print('done with ', others[1][item_num])
